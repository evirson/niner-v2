...
- **⚠️ Data é gravada em UTC, mas sempre COMPARADA no fuso da loja (regra de leitura, 2026-08-19):** as regras de conversão acima cobrem a **gravação** (`TIMESTAMP` → `TIMESTAMPTZ`, armazenado em UTC — isso não muda). Falta a outra metade: a sessão do Postgres roda em `Etc/UTC`, então `CURRENT_DATE`/`now()` crus viram o dia **seguinte** às 21:00 de Brasília. Toda comparação ou agrupamento por data se escreve `(coluna AT TIME ZONE 'America/Sao_Paulo')::date`, e todo "hoje" `(now() AT TIME ZONE 'America/Sao_Paulo')::date` — **dos dois lados**; converter só um continua errado. Colunas que são `date` de verdade ficam de fora (`AT TIME ZONE` sobre `date` não faz sentido). A regra não é teórica: sem ela a loja perdia o caixa aberto às 21h e o controle de horário de acesso consultava o dia da semana errado. Ver `CLAUDE.md` e a linha do tempo de 2026-08-19 em `docs/PROGRESSO.md`.
- **⚠️ O fuso NÃO é `America/Sao_Paulo` — é o da UF da loja (revisão de 2026-08-20):** a regra acima nasceu com a constante de Brasília porque o produto só emitia no Paraná. Como o Nainer é vendido nas **27 unidades da federação** e o Brasil tem **4 fusos**, a constante virou consulta: o fuso do lojista vem da **UF da empresa** (`comum.tempo.FusoDaUf`, identificador IANA — nunca offset, que apodrece se o horário de verão voltar) e o do **plano de controle** é fixo em `America/Sao_Paulo` (`comum.tempo.FusoDaPlataforma`), porque competência de cota, agenda de backup e vigência de assinatura são fatos do negócio da Vetor (P9) e um tenant com empresas em UFs diferentes não teria fuso para desempatar. **Decisão do dono do produto: por UF, sem exceção por município** — Fernando de Noronha (UTC−2, mas é PE) e o extremo oeste do AM (UTC−5, mas é AM) ficam com 1 h de desencontro **conhecido e aceito**, registrado no javadoc; não é bug.
  - **Formatar data também é comparação:** o driver devolve `timestamptz` como `OffsetDateTime` em **UTC**, então `FMT.format(...)` direto imprimia 3 h a mais. Nenhum `DateTimeFormatter` do backend recebe `OffsetDateTime` sem passar por `FusoDaLoja`.
  - **`LocalDate.now()` sem fuso usa o da JVM**, definido só em produção (`TZ` do compose) — o bug não reproduz em dev. `OffsetDateTime.now()` é exceção legítima: representa um instante.
  - **No XML fiscal**, o `dhEmi` (tipo `TDateTimeUTC`) carrega o offset explícito e a SEFAZ compara **instantes** — multi-fuso é suportado pelo padrão; o que ele exige é que o offset seja o da UF do emitente. E a UF de um documento **já emitido** vem congelada na **chave de acesso** (`cUF`, posições 1–2), não da empresa de hoje: documento fiscal é imutável (F6/P3), e é essa UF que escolhe a SEFAZ do cancelamento.
  - **A convenção é verificada, não lembrada:** `ComparacaoDeDataNoFusoCertoTest` varre `src/main/java` e reprova o build.
...
5. **Cobrar por CNPJ** — ⚠️ **esta rejeição foi REVERTIDA em 2026-08-27** pelo dono do produto: a cobrança passa a ser por CNPJ contratado nos planos pagos, com a isenção de 100 vendas somando todos os CNPJs do tenant (o que preserva o incentivo descrito abaixo, já que abrir um segundo CNPJ não custa nada dentro da isenção). Ver `docs/PLANO-DE-NEGOCIO.md`. O texto original da rejeição, mantido para registro: rejeitada explicitamente: filial/CNPJ novo é decisão fiscal, não de volume; cobrar por CNPJ cria o incentivo de espremer tudo num só, que piora o dado e o fiscal. Daí a decisão de **CNPJs ilimitados** com a cota somando todos.
