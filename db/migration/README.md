...
| **V017** | `catalogo`: configs, `cfg_produto_ncm` (referência de NCM, **GLOBAL sem `id_tenant`/RLS** — mantida por script, sem tela, 2026-07-22), `cfg_ean_gerador` + função `gerar_ean13_interno()` (**GLOBAL sem `id_tenant`/RLS**, 2026-07-22 — gerador do código de barras interno de `produto_barra.sku`, sequencial único por instância de banco, `SECURITY DEFINER`), `produto` (sem `imagem`, `codigo_ncm` agora FK para `cfg_produto_ncm`), `produto_categoria` (`indice smallint` único por produto — ordenação da categoria, 2026-07-22), `produto_barra` (**Q7:** `sku` + `ean`; `sku` sempre gerado por `gerar_ean13_interno()`, nunca digitado), `produto_imagem` (galeria, `indice smallint` único por produto) | ✅ |
