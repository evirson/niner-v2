...
-- correto é uma conta de custo/despesa do próprio tenant (o "3.03.001.001 Compra de Mercadoria
-- para Revenda" já reservado no plano padrão, db/scripts/seed_plano_contas_padrao.sql:178-186).
--
-- 1) Semeia a árvore MÍNIMA até essa conta (4 níveis: nivel/id_plano_contas_pai são colunas
...
--    (V016), então inserir os 4 níveis juntos resolve a árvore sem ordem especial.
...
BEGIN
    FOR t_id IN SELECT id_tenant FROM plataforma.tenant LOOP
        ...
        INSERT INTO cfg_plano_contas (
            ...
            (t_id, '3.00.000.000', 'CUSTOS VARIÁVEIS', 'DEBITO', 'SINTETICA',
                true, true, 'CUSTO_VARIAVEL', 'OPERACIONAL', -1, false, false, true),
            (t_id, '3.03.000.000', 'Compras e Custos de Aquisição de Estoque', 'DEBITO', 'SINTETICA',
                false, true, 'NAO_APLICA', 'OPERACIONAL', -1, false, false, true),
            (t_id, '3.03.001.000', 'Aquisição de Mercadoria', 'DEBITO', 'SINTETICA',
                false, true, 'NAO_APLICA', 'OPERACIONAL', -1, false, false, true),
            (t_id, '3.03.001.001', 'Compra de Mercadoria para Revenda', 'DEBITO', 'ANALITICA',
                false, true, 'NAO_APLICA', 'OPERACIONAL', -1, true, false, true)
        ON CONFLICT (id_tenant, id_plano_contas) DO NOTHING;

        UPDATE cfg_geral SET id_plano_contas_compra_mercadoria = '3.03.001.001'
