...
export default function RelatorioVendas() {
  ...
  function textoEmpresasFiltradas(): string {
    if (ehAdmin) {
      return idsEmpresa.length === 0
        ? 'Todas as empresas'
        : (empresas ?? [])
            .filter((e) => idsEmpresa.includes(e.idEmpresa))
  ...
  function montarDescricaoFiltros(): Array<{ rotulo: string; valor: string }> {
    const rotuloPeriodo = PERIODOS.find((p) => p.chave === periodoPreset)?.rotulo
    const periodo = `${dataInicialTexto} a ${dataFinalTexto}` + (periodoPreset !== 'PERSONALIZADO' && rotuloPeriodo ? ` (${rotuloPeriodo})` : '')

    return [
      { rotulo: 'Período', valor: periodo },
      { rotulo: 'Empresa(s)', valor: textoEmpresasFiltradas() },
      { rotulo: 'Vendedor', valor: vendedor ? vendedor.nome : 'Todos os vendedores' },
      { rotulo: 'Totalizar por', valor: TOTALIZADORES.find((t) => t.chave === totalizarPor)?.rotulo ?? '—' },
    ]
  }
