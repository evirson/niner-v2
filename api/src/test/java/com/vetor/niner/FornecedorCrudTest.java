...
class FornecedorCrudTest {
    ...
    void criaFornecedorComDadosCompletos() throws Exception {
        ...
        String fornecedor = """
                {"razaoSocial":"distribuidora abc ltda","idPlanoContas":"2.00.000.000",
    ...
    void cnpjAlfanumericoValidoEhAceito() throws Exception {
        ...
        mvc.perform(post("/api/v1/fornecedores").header("Authorization", "Bearer " + token)
                        .contentType(APPLICATION_JSON)
                        .content("""
                                {"razaoSocial":"Fornecedor Alfanumerico","idPlanoContas":"2.00.000.000",
    ...
    void cnpjComDigitoVerificadorInvalidoEhRejeitado() throws Exception {
        ...
        mvc.perform(post("/api/v1/fornecedores").header("Authorization", "Bearer " + token)
                        .contentType(APPLICATION_JSON)
                        .content("""
                                {"razaoSocial":"CNPJ Errado","idPlanoContas":"2.00.000.000",
    ...
    void cpfNoCampoCnpjEhRejeitado() throws Exception {
        ...
        mvc.perform(post("/api/v1/fornecedores").header("Authorization", "Bearer " + token)
                        .contentType(APPLICATION_JSON)
                        .content("""
                                {"razaoSocial":"CPF Indevido","idPlanoContas":"2.00.000.000",
                                 "cnpj":"111.444.777-35"}
                                """))
                .andExpect(status().isBadRequest());
    }
    ...
    void cnpjDuplicadoEhRejeitado() throws Exception {
        ...
        mvc.perform(post("/api/v1/fornecedores").header("Authorization", "Bearer " + token)
                        .contentType(APPLICATION_JSON)
                        .content("{\"razaoSocial\":\"Primeiro\",\"idPlanoContas\":\"2.00.000.000\",\"cnpj\":\"" + cnpj + "\"}"))
                .andExpect(status().isCreated());

        mvc.perform(post("/api/v1/fornecedores").header("Authorization", "Bearer " + token)
                        .contentType(APPLICATION_JSON)
                        .content("{\"razaoSocial\":\"Segundo\",\"idPlanoContas\":\"2.00.000.000\",\"cnpj\":\"" + cnpj + "\"}"))
    ...
    void telefoneComOitoDigitosEhRejeitado() throws Exception {
        ...
        mvc.perform(post("/api/v1/fornecedores").header("Authorization", "Bearer " + token)
                        .contentType(APPLICATION_JSON)
                        .content("""
                                {"razaoSocial":"Telefone Curto","idPlanoContas":"2.00.000.000",
                                 "telefone":"30554400"}
                                """))
                .andExpect(status().isBadRequest());
    }
    ...
    void campoMarcadoObrigatorioNaConfiguracaoDeTelaEhExigidoNoBackend() throws Exception {
        ...
        mvc.perform(post("/api/v1/fornecedores").header("Authorization", "Bearer " + token)
                        .contentType(APPLICATION_JSON)
                        .content("{\"razaoSocial\":\"Sem Email\",\"idPlanoContas\":\"2.00.000.000\"}"))
                .andExpect(status().isBadRequest());

        mvc.perform(post("/api/v1/fornecedores").header("Authorization", "Bearer " + token)
                        .contentType(APPLICATION_JSON)
                        .content("{\"razaoSocial\":\"Com Email\",\"idPlanoContas\":\"2.00.000.000\",\"email\":\"a@b.com\"}"))
                .andExpect(status().isCreated());
    }
    ...
    void listagemOrdenaPorColunaEDirecaoPedidas() throws Exception {
        ...
        criarFornecedorSimples(token, "2.00.000.000", "ORDFORN BETA");
        criarFornecedorSimples(token, "2.00.000.000", "ORDFORN ALFA");
        criarFornecedorSimples(token, "2.00.000.000", "ORDFORN GAMA");
    ...
    void excluirFornecedorSemVinculoApagaDeVerdade() throws Exception {
        ...
        criarPlano(token, "2.00.000.000");
        long id = criarFornecedorSimples(token, "2.00.000.000", "Fornecedor Sem Vinculo");
    ...
    void excluirFornecedorComMovimentoInativaEmVezDeExcluir() throws Exception {
        ...
        criarPlano(token, "2.00.000.000");
        long id = criarFornecedorSimples(token, "2.00.000.000", "Fornecedor Com Movimento");
