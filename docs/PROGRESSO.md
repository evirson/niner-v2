...
de domínio dos demais módulos (pedido/canais) e o app `admin/`.
...
direto e depois limpo). **Ainda nada commitado** — soma ao volume acumulado do dia inteiro
(PDV completo + fusão `tipo_carteira`/`moeda` + split-tender + estas quatro features).
