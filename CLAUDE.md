...
**Implementation is underway** (see `docs/PROGRESSO.md` for the up-to-date, chronological state). As of 2026-07-22: `api/` (Spring Boot 4 / Java 25) has the control-plane + tenant-domain schema (`db/migration/V001`–`V027`), auth/signup/trial, five full cadastro-pattern screens (`cadastros.cliente` — CRUD + categoria —, `cadastros.funcionario`, `cadastros.planocontas`, `cadastros.fornecedor` and `catalogo.produto` — CRUD + N:N ordered categories + NCM lookup + auto-calculated pricing + oferta rules) plus the first system-config screen (`configuracao.geral`, singleton per tenant, ADMIN-only — deliberately outside the cadastro pattern), and the cross-cutting `comum.telaconfig` (per-tenant field visibility/required); `web/` (React) has the ERP shell, login/handoff, painel, and the Clientes/Fornecedores/Funcionários/Plano de Contas/Produtos/Parâmetros do Sistema screens; `site/` (Astro) has the public landing + signup; `admin/` (backoffice) does not exist yet. The repo also contains:
...
- `docs/PROGRESSO.md` — progress log. `docs/padroes/` — UI reference mockup (golden file, spec §3.7). `docs/telas/` — per-screen feature specs (`cliente.md`, `funcionario.md`, `plano-contas.md`, `fornecedor.md`, `configuracao-geral.md`, `configuracao-tela.md`, `produto.md`); `cliente.md` is the reference for the cadastro screen pattern, `plano-contas.md` shows how to adapt it when the schema differs (business-key PK, no `ativo`), `fornecedor.md` shows the pattern for a mandatory FK to another cadastro (select + embedded quick-create modal, `PlanoContasModal.tsx`), `configuracao-geral.md` shows the pattern for a per-tenant singleton settings screen (no list/create/delete, ADMIN-only), `produto.md` shows the pattern for an N:N ordered child collection (categories), a field controlled by another screen's config (variant names ← Parâmetros do Sistema), and a global no-RLS reference table with no maintenance screen (NCM, `cfg_produto_ncm`).
- `docs/infra/` — cross-cutting infrastructure specs. `armazenamento-imagens.md` (2026-07-23) is the handoff for product-photo object storage: the Google Cloud/Firebase side is **already provisioned and tested**, the Java side **does not exist yet**. Read it before touching `produto_imagem` or writing any upload code — including its credentials section, which is the one part the repo cannot resolve on its own.
